...
interface InstagramFeedJobsClient {
  queue(args: {
    task: string
    input: {
      trigger: 'endpoint'
...
export interface CreateInstagramFeedSyncEndpointOptions {
  path?: string
  taskSlug?: string
  queue?: string
...
export function createInstagramFeedSyncEndpoint(
  options: CreateInstagramFeedSyncEndpointOptions = {},
): PayloadEndpoint {
  const path = normalizeEndpointPath(
    options.path ?? '/dss-instagram-feed/sync',
  )
  const taskSlug =
    options.taskSlug ?? DEFAULT_INSTAGRAM_FEED_TASK_SLUG
  const queue =
    options.queue ?? DEFAULT_INSTAGRAM_FEED_QUEUE
  ...
  return {
    ...
    handler: async (req) => {
      ...
      const jobs =
        req.payload.jobs as unknown as InstagramFeedJobsClient
      ...
      try {
        const job = await jobs.queue({
          ...
          log: [
            {
              message:
                'Instagram feed synchronization queued by protected endpoint.',
              createdAt: queuedAt,
            },
          ],
        })
        ...
        return Response.json(
          {
            error:
              'Unable to queue Instagram feed synchronization.',
          },
          {
            status: 503,
          },
