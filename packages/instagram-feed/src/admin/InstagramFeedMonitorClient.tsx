...
import {
  RegenerateInstagramFeedButton,
  type InstagramFeedMonitorActionState,
...
export interface InstagramFeedMonitorClientProps {
  initialStatus: InstagramFeedAdminStatus
...
export function InstagramFeedMonitorClient({
  initialStatus,
  syncEndpointURL,
  settingsEndpointURL,
  title,
  pollIntervalMs = 1500,
}: InstagramFeedMonitorClientProps) {
  const [status, setStatus] =
    useState(initialStatus)
  const [actionState, setActionState] =
    useState<
      InstagramFeedMonitorActionState
    >('idle')
      ...
      try {
        ...
        if (hasNewRun && latest) {
          ...
          if (
            ...
            setMessage(
              'Job is still queued. Make sure the Instagram feed worker is running.',
            )
          }
        }
      } catch {
  ...
  return (
    <section
      className="dss-instagram-monitor"
      aria-label={title}
      data-sync-endpoint={
        syncEndpointURL
      }
    ...
    >
      <header className="dss-instagram-monitor-header">
        <div>
          <p className="dss-instagram-monitor-eyebrow">
            DSS Feeds
          </p>
          <h2 className="dss-instagram-monitor-title">
            {title}
          </h2>
          <p className="dss-instagram-monitor-description">
            Operational state is
            persisted in Payload and
            refreshed without reloading
            this page. The browser never
            contacts Instagram directly.
          </p>
        </div>
      ...
      <dl className="dss-instagram-monitor-metrics">
        <Metric
          label="Cached posts"
          value={String(
            status.cache
      ...
      {status.cache.warnings.length >
        0 && (
        <div
          className="dss-instagram-monitor-notice"
          role="status"
        >
      ...
      <div className="dss-instagram-monitor-console">
        <div className="dss-instagram-monitor-console-header">
          <span>
            Synchronization log
          </span>
          <span>
        ...
        <div
          className="dss-instagram-monitor-console-body"
          aria-live="polite"
        >
          {events.length > 0 ? (
            events.map(
              (event, index) => (
                <div
                  className="dss-instagram-monitor-log-row"
                  key={`${event.timestamp}-${index}`}
                >
                  <time
                    className="dss-instagram-monitor-log-time"
                    dateTime={
                      event.timestamp
                    }
                  ...
                  <span
                    className={`dss-instagram-monitor-log-level dss-instagram-monitor-log-level--${event.level}`}
                  >
                    {event.level}
                  </span>
                  <span className="dss-instagram-monitor-log-message">
                    {event.message}
                    {event.context && (
                      <code className="dss-instagram-monitor-log-context">
                        {safeStringify(
                          event.context,
                        )}
                      </code>
            ...
            <p className="dss-instagram-monitor-empty">
              No synchronization
              events yet.
            </p>
          )}
      ...
      <p className="dss-instagram-monitor-checked">
        Checked{' '}
        <LocalDate
          value={
            status.checkedAt
...
function Metric({
  ...
  return (
    <div className="dss-instagram-monitor-metric">
      <dt>{label}</dt>
      <dd>{value}</dd>
    </div>
  )
...
function StatusBadge({
  ...
  return (
    <span
      className={`dss-instagram-monitor-status dss-instagram-monitor-status--${state}`}
    >
      {state.replace(
        /_/g,
        ' ',
...
async function fetchRuntimeStatus(
  endpointURL: string,
  previous: InstagramFeedAdminStatus,
...
function parseRuntimeStatus(
  ...
  if (!isRecord(value)) {
    throw new TypeError(
      'Invalid Instagram feed status response.',
    )
  }
...
function withQueuedJob(
  status: InstagramFeedAdminStatus,
): InstagramFeedAdminStatus {
  const timestamp =
...
function resolveCacheState(
  ...
  now: Date,
): InstagramFeedAdminStatus['cache']['state'] {
  if (postCount === 0) {
    return 'empty'
  }
