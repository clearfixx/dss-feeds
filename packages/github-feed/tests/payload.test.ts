...
describe('githubFeedPlugin', () => {
  it(
    'preserves existing config and appends owned Payload schemas',
    async () => {
      const result =
  ...
  it(
    'supports custom slugs and admin grouping',
    async () => {
      const result =
        await githubFeedPlugin({
          settingsSlug:
  ...
  it(
    'fails early on schema slug collisions',
    () => {
      expect(() =>
        githubFeedPlugin({
          settingsSlug:
...
describe(
  ...
  () => {
    it(
      'requires authentication for settings and external cache reads',
      () => {
        expect(
          isAuthenticatedPayloadRequest({
            req: {},
    ...
    it(
      'denies external cache mutations',
      () => {
        expect(
          denyPayloadOperation(),
        ).toBe(false)
...
describe(
  'Payload schema factories',
  () => {
    it(
      'keeps the cache collection hidden and unique by key',
      () => {
        const cache =
          createGitHubFeedCache({
            slug:
    ...
    it(
      'uses safe synchronization defaults',
      () => {
        const settings =
          createSettings()
        const fields =
