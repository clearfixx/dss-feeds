...
describe('fetchGitHubCommits', () => {
  it('normalizes repositories, requests them sequentially, and sorts commits', async () => {
    ...
    const commits = await fetchGitHubCommits(
      {
        username: 'clearfixx',
        repositories: ['portfolio', 'clearfixx/dss-universe'],
        commitLimit: 2,
      },
      {
    ...
    const firstOptions = firstRequest?.[1] as RequestInit
    const headers = new Headers(firstOptions.headers)

    expect(firstUrl.origin).toBe('https://api.github.com')
    expect(firstUrl.searchParams.get('author')).toBe('clearfixx')
    expect(firstUrl.searchParams.get('per_page')).toBe('10')
  ...
  it('adds server-side authentication without exposing it in errors', async () => {
    ...
    expect(caught).toBeInstanceOf(GitHubFeedError)
    expect(String(caught)).not.toContain(token)
    expect((caught as GitHubFeedError).status).toBe(403)
  })
  ...
  it('rejects arbitrary repository URLs before making a request', async () => {
    ...
    await expect(
      fetchGitHubCommits(
        {
          username: 'clearfixx',
          repositories: ['https://example.com/private'],
        },
        {
          fetch: fetchMock as typeof globalThis.fetch,
        },
